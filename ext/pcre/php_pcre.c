...
static void php_do_pcre_match(INTERNAL_FUNCTION_PARAMETERS, int global) /* {{{ */
{
	...
	zend_parse_parameters_new(Z_PARAM_STR(regex) Z_PARAM_STRING(subject, subject_len) Z_OPTIONAL Z_PARAM_ZVAL(subpats) Z_PARAM_LONG(flags) Z_PARAM_LONG(start_offset), {
		RETURN_FALSE;
	})
...
static void preg_replace_impl(INTERNAL_FUNCTION_PARAMETERS, int is_callable_replace, int is_filter)
{
	...
	zend_parse_parameters_new(Z_PARAM_ZVAL(regex) Z_PARAM_ZVAL(replace) Z_PARAM_ZVAL(subject) Z_OPTIONAL Z_PARAM_LONG(limit) Z_PARAM_ZVAL(zcount), {
		return;
	})
...
static PHP_FUNCTION(preg_split)
{
	...
	zend_parse_parameters_new(Z_PARAM_STR(regex) Z_PARAM_STRING(subject, subject_len) Z_OPTIONAL Z_PARAM_LONG(limit_val) Z_PARAM_LONG(flags), {
		RETURN_FALSE;
	})
...
static PHP_FUNCTION(preg_quote)
{
	...
	zend_parse_parameters_new(Z_PARAM_STRING(in_str, in_str_len) Z_OPTIONAL Z_PARAM_STRING(delim, delim_len), {
		return;
	})
...
static PHP_FUNCTION(preg_grep)
{
	...
	zend_parse_parameters_new(Z_PARAM_STR(regex) Z_PARAM_ARRAY(input) Z_OPTIONAL Z_PARAM_LONG(flags), {
		return;
	})
