...
function gen_handler($f, $spec, $kind, $name, $op1, $op2, $use, $code, $lineno, $extra_spec=null) {
	...
	switch($kind) {
		case ZEND_VM_KIND_CALL:
			out($f,"static ZEND_OPCODE_HANDLER_RET ZEND_FASTCALL ".$name.($spec?"_SPEC":"").$prefix[$op1].$prefix[$op2].($spec?extra_spec_name($extra_spec):"")."_HANDLER(ZEND_OPCODE_HANDLER_ARGS)\n");
			break;
		case ZEND_VM_KIND_SWITCH:
			if ($spec) {
				out($f,"case ".((string)($opnames[$name]*25+($typecode[$op1=="TMPVAR"?"TMP":$op1]*5)+$typecode[$op2=="TMPVAR"?"TMP":$op2])).": /*".$name."_SPEC".$prefix[$op1].$prefix[$op2].extra_spec_name($extra_spec)."_HANDLER*/");
			} else {
				out($f,"case ".$name.":");
			}
			if ($use) {
				// This handler is used by other handlers. We will add label to call it.
				out($f," ".$name.($spec?"_SPEC":"").$prefix[$op1].$prefix[$op2].($spec?extra_spec_name($extra_spec):"")."_LABEL:\n");
			} else {
				out($f,"\n");
			}
			break;
		case ZEND_VM_KIND_GOTO:
			out($f,$name.($spec?"_SPEC":"").$prefix[$op1].$prefix[$op2].($spec?extra_spec_name($extra_spec):"")."_HANDLER: ZEND_VM_GUARD(".$name.($spec?("_SPEC".extra_spec_name($extra_spec)):"").$prefix[$op1].$prefix[$op2].");\n");
			break;
	}
...
function gen_labels($f, $spec, $kind, $prolog, &$specs) {
	...
	if ($spec) {
	  ...
	  // For each opcode in opcode number order
		foreach($opcodes as $num => $dsc) {
			...
			if ((count($diff) == count($op_types) - 1 ? isset($diff["ANY"]) : count($diff) != count($op_types)) || isset($dsc["op1"]["TMPVAR"]) || $kind == ZEND_VM_KIND_SWITCH) {
				$spec_op1 = true;
				$specs[$num] .= " | SPEC_RULE_OP1";
			}
			$diff = array_diff_key(array_flip($op_types), isset($dsc["op2"]) ? $dsc["op2"] : []);
			if ((count($diff) == count($op_types) - 1 ? isset($diff["ANY"]) : count($diff) != count($op_types)) || isset($dsc["op2"]["TMPVAR"]) || $kind == ZEND_VM_KIND_SWITCH) {
				$spec_op2 = true;
			...
			$generate = function ($op1, $op2, $extra_spec = []) use ($f, $kind, $dsc, $prefix, $prolog, $num, &$label) {
				global $typecode;
				...
				if (isset($dsc["op1"][$op1]) &&
					...
					switch ($kind) {
						case ZEND_VM_KIND_CALL:
							out($f,$prolog.$dsc["op"]."_SPEC".$prefix[$op1].$prefix[$op2].extra_spec_name($extra_spec)."_HANDLER,\n");
							$label++;
							break;
						case ZEND_VM_KIND_SWITCH:
							// no $extra_spec switch support for now
							out($f,$prolog."(void*)(uintptr_t)".((string)($num*25+$typecode[$op1=="TMPVAR"?"TMP":$op1]*5+$typecode[$op2=="TMPVAR"?"TMP":$op2])).",\n");
							$label++;
							break;
						case ZEND_VM_KIND_GOTO:
							out($f,$prolog."(void*)&&".$dsc["op"]."_SPEC".$prefix[$op1].$prefix[$op2].extra_spec_name($extra_spec)."_HANDLER,\n");
							$label++;
							break;
					}
				} else {
						...
						foreach (extra_spec_handler($opcodes[$num]) as $extra_spec) {
							...
							if (isset($opcodes[$num]["op1"][$op1]) &&
							  ...
							  // Generate handler code
								gen_handler($f, 1, $kind, $opcodes[$num]["op"], $op1, $op2, isset($opcodes[$num]["use"]), $opcodes[$num]["code"], $lineno, $extra_spec);
							}
						}
	...
	foreach ($in as $line) {
		...
		if (strpos($line,"ZEND_VM_HANDLER(") === 0) {
		  // Parsing opcode handler's definition
			...
			// disable extra spec for switch for now
			if (ZEND_VM_SPEC && ZEND_VM_KIND != ZEND_VM_KIND_SWITCH && isset($m[8])) {
